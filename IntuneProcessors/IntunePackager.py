...
class IntunePackager(Processor):
    ...
    def main(self):
        try:
            ...
            print("IntuneWinAppUtil.exe","-c {self.env.get("source_path")}","-s {self.env.get("setup_file")}", "-o self.env.get("destination_path")}", "-q")
            r = subprocess.Popen(["IntuneWinAppUtil.exe","-c {self.env.get("source_path")}","-s {self.env.get("setup_file")}","-o self.env.get("destination_path")}", "-q"])
